...
if($num=="1")
{
	...
	flush();
	$direct='/opt/lampp/htdocs/bnn/';
	$path="youtube-dl -o ".$direct.$vname.".mp4 ".$vvideo." 2>&1";
	//$path = 'python /opt/lampp/htdocs/bnn/execute1.py '.$vname.' '.$vvideo;
    echo exec($path);
}
    ...
    echo str_repeat(' ',512*64);
    flush();  
	$path='python /opt/lampp/htdocs/bnn/execute2.py '.$vname;
	echo exec($path);
	//sleep(1);
if($num=="1")
{
	...
	$path='python /opt/lampp/htdocs/bnn/execute3.py '.$vname;
	exec($path);
...
{	$percent='30%';
    ...
    echo str_repeat(' ',512*64);
    flush();
   $path='python /opt/lampp/htdocs/bnn/execute4.py '.$vname.' '.$num;
	exec($path); 
	//sleep(1);
if($num=="1")
{	
...
else
{
    ...
    echo str_repeat(' ',512*64);
    flush(); 
	$path='python /opt/lampp/htdocs/bnn/execute5.py '.$vname.' '.$num;
	exec($path);
	sleep(20);
